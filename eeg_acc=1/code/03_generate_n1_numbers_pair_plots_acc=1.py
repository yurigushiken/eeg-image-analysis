...
def generate_n1_pair_plots():
    ...
    for inc_cond, dec_cond in NUMBER_PAIRS:
        print(f"\n--- Generating N1 group plot for {inc_cond} vs {dec_cond} (ACC=1) ---")
        ...
        script_name = f"03_generate_n1_numbers_pair_plot_inc{inc_cond}_vs_dec{dec_cond}_acc=1"
        ...
        fig_grp.suptitle(f'Grand Average: N1 ({inc_pair_str} vs {dec_pair_str}, ACC=1)', fontsize=16)
        ...
        mne.viz.plot_compare_evokeds(
            grand_averages_key, picks=N1_ELECTRODES, combine='mean', axes=ax_erp_grp,
            title='Grand Average Mean ERP over Bilateral Posterior-Occipital Region', show=False, legend='upper left', ci=0.95,
            colors=plot_colors
        )
        ...
        for i, (cond_name, evoked) in enumerate(grand_averages_key.items()):
            try:
                roi_data = evoked.get_data(picks=N1_ELECTRODES).mean(axis=0)
        ...
        plt.close(fig_grp)
        print(f"  - Saved group N1 plot to {grp_fig_path}")

if __name__ == '__main__':
    generate_n1_pair_plots() 