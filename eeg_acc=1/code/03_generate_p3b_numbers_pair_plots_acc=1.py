...
def generate_p3b_pair_plots():
    ...
    for inc_cond, dec_cond in NUMBER_PAIRS:
        print(f"\n--- Generating P3b group plot for {inc_cond} vs {dec_cond} (ACC=1) ---")
        ...
        script_name = f"03_generate_p3b_numbers_pair_plot_inc{inc_cond}_vs_dec{dec_cond}_acc=1"
        ...
        fig_grp.suptitle(f'Grand Average: P3b ({inc_pair_str} vs {dec_pair_str}, ACC=1)', fontsize=16)
        ...
        mne.viz.plot_compare_evokeds(
            ...
            title='Grand Average Mean ERP over Centro-Parietal Region', show=False, legend='upper left', ci=0.95,
            colors=plot_colors
        )
        ...
        for i, (cond_name, evoked) in enumerate(grand_averages_key.items()):
            try:
                roi_data = evoked.get_data(picks=P3B_ELECTRODES).mean(axis=0)
